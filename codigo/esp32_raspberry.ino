...
#include <TinyGPS++.h>  // Librería para procesar los datos del módulo GPS

// Crear el objeto GPS
TinyGPSPlus gps;

// --- PINES DEL GPS ---
// Conectar TX del GPS al 16, RX del GPS al 17
...
void setup() {
  ...
  // Iniciar comunicación serial con el módulo GPS a 9600 baudios
  ...
  // Configurar el módulo PWM de la ESP32 al pin ENA (Sintaxis actualizada)
  ...
  Serial.println("INFO: ESP32 Inicializada. Puente H, ESC y GPS listos.");
}
...
void loop() {
  ...
  // --- LECTURA DEL MÓDULO GPS ---
  while (Serial2.available() > 0) {
    char c = Serial2.read();
    if (gps.encode(c)) {
      if (gps.location.isValid() && gps.location.isUpdated()) {
        // Enviar coordenadas a la Raspberry Pi
...
void ejecutarComando(char comando, int potenciaPorcentaje) {
  ...
  switch (comando) {
    ...
    case 'B': // Encender Banda Recolectora
      ...
      // porque Python manda un 0 en el porcentaje cuando manda la 'B'
      encenderBanda(200); 
...
void encenderBanda(int velocidadPWM) {
  ...
  // Enviar el valor PWM indicando el PIN en lugar del canal
...
void detenerBanda() {
  ...
  // Cortar el voltaje PWM usando el PIN
